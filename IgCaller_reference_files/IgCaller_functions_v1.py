...
def predefinedFilter(information, GENE, tumorPurity):
	...
	for line in information:
		...
		if "Kde" not in line[0] and "RSS" not in line[0]:
			if sum(1 for i in line[12] if i == "N")/len(line[12]) > 0.5: continue # remove rearrangement if >50% of the V sequence are "N"s
...
def getIgTranslocations(pathToSamtools, threadsForSamtools, bamT, bamN, chrom, coordsToSubset, tumorPurity, mntonco, mntoncoPass, mnnonco):
	...
	comms = pathToSamtools+"samtools view -@ "+threadsForSamtools+" "+bamT+" > "+samT 
	subprocess.call(comms, shell=True)
	
	if bamN is not None:
		samN = bamN.replace(".bam", ".sam")
		comms = pathToSamtools+"samtools view -@ "+threadsForSamtools+" "+bamN+" > "+samN 
		subprocess.call(comms, shell=True)
