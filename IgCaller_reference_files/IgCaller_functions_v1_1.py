...
def predefinedFilter(information, GENE, tumorPurity, seq):
	...
	for line in information:
		...
		elif "Kde" in line[0] and "RSS" in line[0]: # remove Kde-RSS if <2 split reads and <4 insertSize reads
		...
		if len(trip) == 0:
			...
			if line[0] in trip: 
				...
				if "Kde" not in line[0] and "RSS" not in line[0]:
					...
					if int(line[2]) == 0 and int(trip[line[0]][2]) == 0 and spl_ins >= int(trip[line[0]][1]): # both 0 split reads, keep new - overwrite previous if >= score
						trip[line[0]] = [line[1], spl_ins]+line[2:]
					elif int(line[2]) > 0 and int(trip[line[0]][2]) == 0: # if new has split reads, keep new, overwrite previous
						trip[line[0]] = [line[1], spl_ins]+line[2:]
					elif int(line[2]) < 2 and int(trip[line[0]][2]) > 1: # if new has <2 split reads, keep old
						continue
					elif int(line[2]) > 1 and int(trip[line[0]][2]) < 2: # if new has 2+ split reads and old <2, keep new, overwrite previous
						trip[line[0]] = [line[1], spl_ins]+line[2:]
					else: # keep both
			...
			# if not exact VDJ...
				...
				for keys in [k for k in trip]: # make list of keys to avoid dictionary changed size during iteration
					...
					if len(common) > 1: 
						...
						# if len == 2 in both...
							...
							del trip[keys]
							trip[line[0]] = [line[1], spl_ins]+line[2:]
						elif "Productive" in trip[keys][17] and "Productive" not in line[17]: # if annotated is productive
							pr = 1 
				...
				if pr == 0: # not present in dictionary
					trip[line[0]] = [line[1], spl_ins]+line[2:]
