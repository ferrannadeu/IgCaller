				...
				if line[19] != "NA" and line[19] in [trip[keys][18] for keys in trip]:
					for keys in [k for k in trip]: # make list of keys to avoid dictionary changed size during iteration
						...
						if line[19] == trip[keys][18]:
							if spl_ins >= dict_spl_ins-10-cutoffScore and spl_ins <= dict_spl_ins+10+cutoffScore and spl_ins > 10+cutoffScore and dict_spl_ins > 10+cutoffScore: # if similar scores and higher than 10+cutoffScore in both, filter based on quality
								if mq > dict_mq + 10:
									del trip[keys]
									...
									pr = 1
							elif spl_ins > dict_spl_ins: # different score or one lower than 10+cutoffScore, keep highest score
								del trip[keys]
							else:
								pr = 1
